...
const db = {
	...
	state: { 1: false, 2: false, 3: false, clickToFlip: true },
};
...
function server() {
	...
	app.get("/flashcard-blocks", (req, res) =>
		res.json({ flashcardBlocks: db.flashcardBlocks, flashcardFlippedState: db.state })
	);
	app.get("/knowledge-check-blocks", (req, res) => res.send(db.knowledgeCheckBlocks));

	app.post("/flashcard-flipped-state", (req, res) => {
		db.state = req.body;
